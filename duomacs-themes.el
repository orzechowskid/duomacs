...
(require 'duomacs-nord-theme)
(require 'duomacs-nord-light-theme)


(defcustom duomacs-use-system-theme
	t
	"Whether or not to honor the OS' light/dark theme.")
...
;;   :group 'duomacs
;;   :type '(choice
;;           (const :tag "Duomacs Nord Light" duomacs-nord-light)
;;           (const :tag "Duomacs Nord Dark" duomacs-nord))
;;   :set (lambda (sym val)
;; 	 (if (boundp sym)
;; 	     ;; changing value
...
;; 	   (unless duomacs-use-system-theme
;; 	     (set-default-toplevel-value sym val)
;; 	     (if (functionp val)
;;                  (funcall val)
;;                (load-theme val))))))
